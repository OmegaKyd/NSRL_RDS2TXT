...
3. Leave `..._NSRLFile.txt`, `..._MD5.txt`, and `..._SHA1.txt` checked for AXIOM / GID Manager.
4. Click Convert.
...
| `*_MD5.txt` | One uppercase MD5 per line, no header | AXIOM and GID Manager. Import as MD5 / Hex. |
| `*_SHA1.txt` | One uppercase SHA-1 per line, no header | AXIOM and GID Manager. Import as SHA1 / Hex. |
...
Do not import `*_NSRLFile.txt` into GID Manager or AXIOM's simple hash-list picker. Those tools treat each line as one hash and will mis-detect the multi-column file as SHA-512 / Base64. Use the MD5 or SHA1 list instead.

The files are quoted CSV with a `.txt` extension, which is the official RDS 2.xx layout. There is an option to also write `.csv` copies of the same content.
...
- Hash import tools (GID Manager, X-Ways, EnCase, FTK, and similar) read `NSRLFile.txt` by header name or by the fixed RDS 2.xx column order. Extra columns in that file are normal.
